...
-- V052: Audit Dispatcher Functional Implementation
    ...
    CASE v_op
        WHEN 'list_logs' THEN
            SELECT jsonb_agg(l) INTO v_res FROM (
                ...
                WHERE ((v_data->>'entityType') IS NULL OR al.entity_type = (v_data->>'entityType'))
                  AND ((v_data->>'entityId') IS NULL OR al.entity_id = (v_data->>'entityId')::UUID)
